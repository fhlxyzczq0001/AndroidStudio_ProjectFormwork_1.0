...
public class Business_SharePer_GlobalInfo {
    //初始化SharedPreferences
    private static SharePre sharePre = null;
    private static Business_SharePer_GlobalInfo single = null;

    private Business_SharePer_GlobalInfo() {
        getSharePre();
    }

    //静态工厂方法
    public static Business_SharePer_GlobalInfo getInstance() {
        if (single == null || sharePre == null) {
            single = new Business_SharePer_GlobalInfo();
        }
        return single;
    }
